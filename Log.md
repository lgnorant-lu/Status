...
- [2025-05-14] [核心系统] 实现错误恢复机制 - 详见 [Logs/core/2025-05-14_core_recovery_system.md]
  - 开发StateManager用于应用状态持久化管理，支持状态保存、加载、校验和修复
  - 实现RecoveryManager处理应用非正常终止情况，提供三种恢复模式(正常/安全/最小)
  - 创建ExceptionHandler提供全局异常捕获和处理，支持四级错误级别(致命/严重/中度/轻微)
  - 设计单例模式确保核心组件全局唯一，添加完整的单元测试与使用示例
