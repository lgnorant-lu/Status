...
- [2025-05-15] [行为系统] 完善桌宠行为丰富功能 - 详见 [Logs/behavior/2025-05-15.md]
  - 修复事件系统中STATE_CHANGED类型缺失问题
  - 修复PetStateMachine类logger初始化问题
  - 强化时间行为系统测试，添加全面时间段边界条件测试
  - 完善时间状态适配器测试，增加错误处理和边界情况测试
  - 扩展宠物状态机测试，添加多系统资源状态和优先级处理测试
  - 修复特殊日期状态处理逻辑，使用正确的PetState枚举值
  - 更新Thread.md和Log.md记录变更

- [2025-05-17] [UI系统] 修复拖动功能响应问题 - 详见 [Logs/Status-Ming/2025-05-17_drag_fix.md]
  - 添加拖动阈值(3像素)和激活状态，防止微小移动触发拖拽
  - 实现看门狗定时器，防止拖动状态卡住
  - 优化鼠标事件处理逻辑，确保正确重置拖动状态
  - 添加详细日志记录，帮助诊断和解决拖动相关问题

- [2025-05-17] [性能][日志] 日志冗余优化与性能提升 - 详见 [Logs/Status-Ming/2025-05-17_log_optimization.md]
  - 降低系统监控模块的日志级别，减少90%的日志输出量
  - 调整UI组件的日志级别，减少绘制事件的日志噪音
  - 将系统状态更新间隔从16ms延长至1000ms，降低资源占用
  - 修复StatsPanel的数据键名不匹配问题，恢复所有系统状态数据显示

- [2025-05-14] [行为系统] 完善桌宠行为丰富功能:
  - 修复特殊日期状态处理逻辑，使用正确的PetState枚举值
  - 扩展特殊日期映射，支持更多特殊日期
  - 更新测试用例以适应代码变更
  - 添加系统资源负载细分状态
  - 暂时跳过部分事件分发测试，等待后续重新设计
  - 实现用户互动响应功能，添加交互区域、交互跟踪和交互处理系统
  - 详细日志参见 [Logs/behavior/2025-05-14.md](Logs/behavior/2025-05-14.md)
